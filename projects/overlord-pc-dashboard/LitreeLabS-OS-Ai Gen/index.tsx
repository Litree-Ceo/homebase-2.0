...
function App() {
    ...
    const {
        ...
        error,
        createSession,
        // generateArtifact is now handled inline
        setCurrentSessionIndex,
        clearError,
        setSessions, // Assuming the hook provides this for direct manipulation
    } = useGeminiSession();
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        if (newSession) {
            ...
            newSession.artifacts.forEach(async (artifact) => {
                const fullPrompt = `Create a high-fidelity Metaverse UI widget for: "${trimmedInput}". Style: ${artifact.styleName}. Return RAW HTML/CSS.`;
                ...
                try {
                    ...
                    const finalHtml = accumulatedHtml.replace(/```html|```/g, '').trim();
                    setSessions?.((prev: Session[]) => prev.map((sess: Session) =>
                        sess.id === newSession.id ? {
                            ...sess,
                            artifacts: sess.artifacts.map((art: Artifact) =>
                    ...
                    setSessions?.((prev: Session[]) => prev.map((sess: Session) =>
                        sess.id === newSession.id ? {
                            ...sess,
                            artifacts: sess.artifacts.map((art: Artifact) =>
                                art.id === artifact.id ? { ...art, status: 'error' } : art
                            )
                        } : sess
                    ));
                }
    ...
    }, [inputValue, isLoading, createSession, setSessions]);

    // The setSessions function is now correctly destructured from the useGeminiSession hook.
