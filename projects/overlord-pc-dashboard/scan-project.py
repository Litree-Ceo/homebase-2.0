...
def main():
    ...
    # Check if API key is configured (from env or config)
    try:
        api_key = os.getenv("OVERLORD_API_KEY") or config.get("auth", {}).get("api_key", "")
        if not api_key or "change" in api_key.lower() or "default" in api_key.lower() or len(api_key) < 20:
            print(f"  [WARN] API key looks weak/default - consider changing")
        else:
            print(f"  [OK] API key appears configured ({len(api_key)} chars)")
    except Exception as e:
        print(f"  [ERR] Could not check API key: {e}")

    # Check Real-Debrid API key
    rd_api_key = os.getenv("RD_API_KEY", "")
    if rd_api_key and "your-" not in rd_api_key.lower() and len(rd_api_key) > 10:
        print(f"  [OK] Real-Debrid API key configured ({len(rd_api_key)} chars)")
    else:
        # Fallback: check .env file directly
        if os.path.exists(".env"):
            with open(".env") as f:
                env_content = f.read()
            if "RD_API_KEY=" in env_content and "your-" not in env_content.lower():
