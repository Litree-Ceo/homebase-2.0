...
# LITLAB PLAYER BRIDGE v1.0 — auth-first, heartbeat, action dispatcher
# ws://localhost:8765  (falls back to next free port if occupied)

AUTH_TOKEN = os.getenv("LITLAB_AUTH_TOKEN")
if not AUTH_TOKEN:
    raise RuntimeError("LITLAB_AUTH_TOKEN environment variable is required")
...
async def handler(websocket, _path=None):
    ...
    try:
        ...
        await websocket.send(
            json.dumps({"type": "auth_ok", "msg": "LITLAB BRIDGE ONLINE"})
        )
...
async def main():
    try:
        ...
        async with websockets.serve(handler, "localhost", port):
            print(f"[BRIDGE] ✅ Litlab Player Bridge ONLINE → ws://localhost:{port}")
            print(
