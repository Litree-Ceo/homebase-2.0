...
export async function getAIMemorySummary(uid: string): Promise<string[]> {
  ...
  if (context.aiPreferences.favoriteHashtags?.length) {
    memories.push(
      `#️⃣ Favorite hashtags: ${context.aiPreferences.favoriteHashtags.slice(0, 3).join(", ")}`,
    );
  }
  ...
  if (context.history.mostUsedPrompts?.length) {
    ...
    memories.push(
      `⭐ Most used: "${top.prompt.slice(0, 40)}..." (${top.count}x)`,
    );
  }

  return memories.length
    ? memories
    : ["No AI memory yet - start generating content!"];
}
