...
export async function getSmartRecommendations(uid: string): Promise<string[]> {
  ...
  if (tier === "free") {
    ...
    if (usage.aiGenerations >= limits.aiGenerations - 1) {
      recommendations.push(
        "💎 You're almost at your daily limit! Upgrade to Pro for unlimited AI generations.",
      );
    }

    if (usage.dmReplies >= limits.dmReplies) {
      recommendations.push(
        "📱 Out of DM replies for today. Pro members get unlimited smart replies.",
      );
    }
  }
  ...
  if (usage.aiGenerations > 10) {
    recommendations.push(
      "🔥 You're on fire today! Consider scheduling these posts throughout the week.",
    );
  }

  if (usage.moneyPlays === 0) {
    recommendations.push(
      "💰 Don't forget to generate a money play today - it could boost your revenue by 25%!",
    );
  }
