...
Respond helpfully. If you detect an upsell opportunity, mention it naturally. If the question is complex or the user seems frustrated, suggest escalating to human support.`;
  ...
  try {
    ...
    return {
      message:
        "I'm having trouble connecting right now. Please try again in a moment, or email us at support@litlabs.ai",
      intent: "support",
...
function detectIntent(
  ...
  if (
    response.toLowerCase().includes("upgrade") ||
...
export async function trackSparkConversation(
  ...
  console.log("Spark conversation tracked:", {
    userId,
    helpful,
