...
type SpeechRecognitionType = typeof window extends {
  ...
  ? T
  : any;
type WebkitSpeechRecognitionType = typeof window extends {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hey! 👋 I'm SPARK, your AI support assistant. Need help with anything?",
      timestamp: new Date(),
    },
  ]);
    ...
    const SpeechRecognition =
      ...
      (
        window as unknown as {
          webkitSpeechRecognition?: WebkitSpeechRecognitionType;
        }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.escalateToHuman) {
        setTimeout(() => {
          const escalationMessage: Message = {
            role: "assistant",
            content:
              "I've escalated your issue to our support team. They'll reach out within 24 hours! 🙌",
            timestamp: new Date(),
          };
      ...
      const errorMessage: Message = {
        role: "assistant",
        content:
          "Oops! Something went wrong. Try again or email us at support@litlabs.ai",
        timestamp: new Date(),
      };
        ...
        <div className="fixed bottom-6 right-6 z-50 w-96 h-[600px] rounded-2xl border border-white/10 bg-slate-900 shadow-2xl flex flex-col">
          ...
          <div className="p-4 border-t border-white/10">
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-3 py-3 rounded-xl border border-white/10 bg-white/5 text-white transition ${
                  isListening
                    ? "border-pink-500/60 shadow-[0_0_0_3px_rgba(236,72,153,0.25)]"
                    : "hover:border-pink-500/50"
                } disabled:opacity-50 disabled:cursor-not-allowed`}
                title={
                  voiceSupported
                    ? "Click to capture speech"
            ...
            <p className="text-[10px] text-white/40 mt-2 text-center">
              Press <kbd className="px-1 py-0.5 rounded bg-white/10">Enter</kbd>{" "}
              to send •{" "}
