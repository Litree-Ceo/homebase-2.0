...
export function LabEmulationHub() {
  const items = [
    {
      title: "Retro library",
      detail: "NES / SNES / PS1 / N64 covers with holo glow.",
    },
    {
      title: "Cloud saves",
      detail: "State sync across devices; premium storage tiers.",
    },
    {
      title: "Streaming overlay",
      detail: "One-click Twitch/YouTube overlay with neon UI.",
    },
    {
      title: "Challenges",
      detail: "Leaderboards + badges for retro runs; paid events.",
    },
  ];
    ...
    <section className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_0_30px_rgba(16,185,129,0.12)]">
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-emerald-200">
        <span className="rounded-full bg-emerald-500/10 px-3 py-1">
          Premium emulation packs
        </span>
        <span className="rounded-full bg-emerald-500/10 px-3 py-1">
          Streaming boost add-on
        </span>
        <span className="rounded-full bg-emerald-500/10 px-3 py-1">
          Voice: “Launch X”
        </span>
