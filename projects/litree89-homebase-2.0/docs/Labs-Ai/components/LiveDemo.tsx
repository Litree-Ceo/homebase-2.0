...
export default function LiveDemo() {
  ...
  async function send() {
    ...
    try {
      ...
      if (demoToken && demoToken.length > 0)
        headers["x-demo-token"] = demoToken;

      // If reCAPTCHA site key is present, attempt to get a token and include it
      const siteKey = process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY as unknown as
        | string
        | undefined;
      ...
      if (siteKey) {
        ...
        if (w.grecaptcha && w.grecaptcha.execute) {
          try {
            recaptchaToken = await w.grecaptcha.execute(siteKey, {
              action: "demo",
            });
          } catch {
  ...
  return (
    <div className="mt-4 rounded-lg border border-slate-800 bg-slate-900/60 p-4 text-sm">
      <p className="mb-2 text-xs text-slate-400">
        Try a quick demo — no signup required (rate-limited).
      </p>
      <textarea
        value={message}
