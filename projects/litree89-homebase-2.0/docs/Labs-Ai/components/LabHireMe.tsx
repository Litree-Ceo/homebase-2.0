...
export function LabHireMe() {
  ...
  return (
    <section
      id="hire-me"
      className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_0_30px_rgba(16,185,129,0.12)]"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="text-xl font-semibold text-white">
            Google Business + direct booking
          </h3>
          <p className="text-sm text-slate-300 max-w-2xl">
            Need custom builds, overlays, or integrations? Tap a contact
            route—email, call, or schedule. Availability updates in real time;
            AI assistant can route the request.
          </p>
        </div>
      ...
      <div className="mt-4 rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100">
        AI shortcut: “Reach out to the owner” → auto-opens contact form or
        schedules a call.
      </div>
    </section>
  );
