...
export async function POST(req: NextRequest) {
  try {
    ...
    const getTierFromPriceId = (priceId: string): string => {
      ...
      if (priceId === process.env.NEXT_PUBLIC_STRIPE_DELUXE_PRICE_ID)
        return "deluxe";
      return "free";
    };
    ...
    switch (event.type) {
      ...
      case "invoice.payment_failed": {
        ...
        try {
          ...
          console.log(
            `Payment failed notification sent for invoice: ${invoice.id}`,
          );
        } catch (error) {
          const err = error as Error;
      ...
      case "invoice.payment_succeeded": {
        ...
        try {
          ...
          if (invoiceData.subscription) {
            console.log(
              `Payment succeeded for subscription: ${invoiceData.subscription}`,
            );
