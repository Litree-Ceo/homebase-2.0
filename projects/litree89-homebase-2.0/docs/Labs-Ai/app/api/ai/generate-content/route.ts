...
export async function POST(req: NextRequest) {
  try {
    ...
    const niche = isStringArrayMember(allowedNiches, parsed.niche)
      ? parsed.niche
      : undefined;
    const contentType = isStringArrayMember(
      allowedContentTypes,
      parsed.contentType,
    )
      ? parsed.contentType
      : undefined;
    const tone = isStringArrayMember(allowedTones, parsed.tone)
      ? parsed.tone
      : undefined;
    ...
    if (uid) {
      ...
      if (!securityCheck.safe && securityCheck.threat) {
        ...
        return NextResponse.json(
          {
            error:
              "Security check failed. Please contact support if you believe this is an error.",
          },
          { status: 403 },
        );
      }
