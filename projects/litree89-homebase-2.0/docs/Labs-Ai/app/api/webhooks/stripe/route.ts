  ...
  try {
    ...
    switch (type) {
      case "checkout.session.completed": {
        ...
        if (!userId || !email) {
          warn(
            "[Stripe Webhook] Missing userId or email in checkout session metadata",
          );
        ...
        info(
          `✅ Stripe: ${email} subscribed to ${determinedTier} - Subscription ${subscription}`,
        );
        break;
      }
        ...
        if (!userId) {
          ...
          if (db) {
            ...
            if (!usersSnap.empty) {
              ...
              info(
                `✅ Stripe: Subscription updated - ${userDoc.data().email} → ${tier} (${status})`,
              );
            }
          }
        } else {
          ...
          info(
            `✅ Stripe: Subscription updated - User ${userId} → ${tier} (${status})`,
          );
        }
        break;
      }
      ...
      case "customer.subscription.deleted": {
        ...
        if (db) {
          if (userId) {
            ...
            info(
              `✅ Stripe: Subscription canceled - User ${userId} downgraded to free`,
            );
          } else {
            // Find by customer ID
            const usersSnap = await db
            ...
            if (!usersSnap.empty) {
              ...
              info(
                `✅ Stripe: Subscription canceled - ${userDoc.data().email} downgraded to free`,
              );
            }
          }
        }
      ...
      case "invoice.payment_succeeded": {
        const invoiceObj = data.object as any;
        const { customer, customer_email, subscription, amount_paid } =
          invoiceObj;
        ...
        if (db) {
          ...
          if (!usersSnap.empty) {
            ...
            info(
              `✅ Stripe: Payment succeeded for ${userDoc.data().email} - $${amount_paid / 100}`,
            );
          }
        }
        break;
