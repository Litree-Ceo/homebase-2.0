...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expectedSecret || !webhookSecret) {
      error(
        "❌ Unauthorized subscription-update attempt - missing webhook secret",
      );
    ...
    if (receivedBuffer.length !== expectedBuffer.length) {
      error(
        "❌ Unauthorized subscription-update attempt - invalid webhook secret length",
      );
      return NextResponse.json(
        { error: "Forbidden - This endpoint is for internal webhook use only" },
        { status: 403 },
    ...
    if (!crypto.timingSafeEqual(receivedBuffer, expectedBuffer)) {
      error(
        "❌ Unauthorized subscription-update attempt - webhook verification failed",
      );
      return NextResponse.json(
        { error: "Forbidden - This endpoint is for internal webhook use only" },
        { status: 403 },
