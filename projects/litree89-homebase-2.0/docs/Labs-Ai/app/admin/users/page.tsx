...
export default function AdminUsersPage() {
  ...
  function Filters({
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {/* Tier Filter */}
        <div>
          <label htmlFor="filter-tier" className="block text-white/60 text-sm font-medium mb-2">
            Tier
          </label>
          <select
            id="filter-tier"
        ...
        {/* Status Filter */}
        <div>
          <label htmlFor="filter-status" className="block text-white/60 text-sm font-medium mb-2">
            Status
          </label>
          <select
            id="filter-status"
  ...
  function UsersTable({
    ...
    users: User[];
    allUsers: User[];
    onUpgradeTier: (uid: string, newTier: 'free' | 'pro' | 'enterprise') => void;
    onToggleSuspension: (uid: string, currentStatus: string) => void;
          ...
          <table className="w-full">
            ...
            <tbody>
              {users.length > 0 ? (
                users.map((user) => (
                  <tr key={user.uid} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${user.tier === 'free' ? 'bg-blue-500/20 text-blue-400' : user.tier === 'pro' ? 'bg-purple-500/20 text-purple-400' : 'bg-orange-500/20 text-orange-400'}`}>{user.tier}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${user.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{user.status}</span>
                    </td>
                    <td className="px-6 py-4 text-white/60 text-sm">{user.postsThisMonth || 0} / {user.totalPosts || 0}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        {user.tier !== 'pro' && (
                          <button onClick={() => onUpgradeTier(user.uid, 'pro')} className="px-3 py-1 rounded text-xs font-medium bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 transition-colors">to Pro</button>
                        )}
                        {user.tier !== 'enterprise' && (
                          <button onClick={() => onUpgradeTier(user.uid, 'enterprise')} className="px-3 py-1 rounded text-xs font-medium bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 transition-colors">to Ent.</button>
                        )}
                        <button onClick={() => onToggleSuspension(user.uid, user.status)} className={`px-3 py-1 rounded text-xs font-medium transition-colors ${user.status === 'active' ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-green-500/20 text-green-400 hover:bg-green-500/30'}`}>{user.status === 'active' ? 'Suspend' : 'Activate'}</button>
                        <button onClick={() => onDeleteUser(user.uid, user.email)} className="px-3 py-1 rounded text-xs font-medium bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-8 text-center text-white/40 text-sm">No users found</td>
                </tr>
              )}
            </tbody>
          </table>
  ...
  export default function AdminUsersPage() {
      ...
      const unsub = onAuthStateChanged(authInstance, async (user) => {
        ...
        const unsubUsers = onSnapshot(usersQuery, (snapshot) => {
          ...
          snapshot.forEach((doc) => {
            usersList.push({ uid: doc.id, ...(doc.data() as Record<string, unknown>) } as User);
          });
          setUsers(usersList);
          setFilteredUsers(applyFilters(usersList, searchTerm, filterTier, filterStatus));
        });
        setLoading(false);
        return () => unsubUsers();
      });
    ...
    // Filtering logic
    function applyFilters(usersList: User[], search: string, tier: string, status: string) {
    ...
    const handleSearch = (value: string) => {
      setSearchTerm(value);
      setFilteredUsers(applyFilters(users, value, filterTier, filterStatus));
    };
    const handleTierFilter = (value: string) => {
      setFilterTier(value as 'all' | 'free' | 'pro' | 'enterprise');
      setFilteredUsers(applyFilters(users, searchTerm, value, filterStatus));
    };
    const handleStatusFilter = (value: string) => {
      setFilterStatus(value as 'all' | 'active' | 'suspended');
      setFilteredUsers(applyFilters(users, searchTerm, filterTier, value));
    };
    const upgradeTier = async (uid: string, newTier: 'free' | 'pro' | 'enterprise') => {
      try {
        const dbRef = db;
        if (!dbRef) {
