    ...
    <section className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_0_30px_rgba(16,185,129,0.12)]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="text-xl font-semibold text-white">
            Play with layout toggles
          </h3>
          <p className="text-sm text-slate-300">
            Swap themes, density, and immersive mode. Full drag-and-drop can
            hook into this shell.
          </p>
        </div>
        <div className="flex flex-wrap gap-2 text-sm">
          <button
            ...
            className={`rounded-full px-3 py-1 border text-emerald-200 transition ${
              immersion
                ? "border-emerald-300 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.4)]"
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {widgets.map((w) => (
          ...
          >
            <div
              className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${accentClass} opacity-15`}
            />
            ...
            <p className="relative mt-2 text-sm text-slate-300">
              {immersion
                ? "Immersive glow + particles preview."
                : "Standard neon tile preview."}
            </p>
          </div>
        ))}
      </div>
