---
# 🤖 Copilot Instructions for EverythingHomebase
...
- **Tech stack**: TypeScript, Next.js 14 (App Router), Firebase (Firestore/Auth/Analytics), Stripe, **Azure AI (OpenAI, Copilot, Azure Cognitive Services)**, PowerShell scripts for automation.
...
- **Theme system**: Add themes to `config/themes.ts`. Use `components/theme/ThemeSwitcher.tsx` to preview/apply themes. See `dashboard/honeycomb/` for layout and style reference.
...
- **AI personas**: All AI features should use Azure OpenAI or Azure Copilot as the primary model/context provider. Add/modify in `app/api/ai/chat/route.ts` and `config/widgets.ts`. For new AI features, prefer Azure-hosted endpoints and OpenAI APIs via Azure.
...
- **Switch dashboard theme**: Use the ThemeSwitcher component in your dashboard or layout to let users pick a Honeycomb Vision theme.
- **Honeycomb layout**: Reference `dashboard/honeycomb/index.html` and `style.css` for the hex grid dashboard pattern.
