...
export function LabPreferences() {
  const { intensity, particles, sound, setIntensity, setParticles, setSound } =
    usePreferences();
    ...
    <section className="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-[0_0_30px_rgba(16,185,129,0.12)]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="text-xl font-semibold text-white">
            Performance & immersion toggles
          </h3>
          <p className="text-sm text-slate-300">
            Let visitors tune visuals and audio. These can map to saved
            preferences later.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3 text-sm text-slate-200">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3">
          ...
          <p className="mt-2 text-xs text-emerald-200">
            Tie this to actual audio hooks later; default muted for
            accessibility.
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100">
        Current preference state: intensity={intensity}, particles=
        {particles ? "on" : "off"}, sound={sound ? "on" : "off"}. Replace with
        persisted user settings when wiring to backend.
      </div>
    </section>
  );
}
