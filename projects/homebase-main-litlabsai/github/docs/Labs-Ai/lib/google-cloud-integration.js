...
class GoogleCloudIntegration {
  ...
  async initializeBigQuery() {
    try {
      ...
      await this.createTableIfNotExists(dataset, "events", {
        schema:
          "user_id:STRING, event_type:STRING, event_data:JSON, timestamp:TIMESTAMP, source:STRING",
      ...
      await this.createTableIfNotExists(dataset, "payments", {
        schema:
          "payment_id:STRING, user_id:STRING, amount:NUMERIC, currency:STRING, status:STRING, stripe_id:STRING, created_at:TIMESTAMP, updated_at:TIMESTAMP",
      ...
      await this.createTableIfNotExists(dataset, "automations", {
        schema:
          "automation_id:STRING, user_id:STRING, type:STRING, trigger:STRING, action:STRING, status:STRING, executions:INTEGER, last_run:TIMESTAMP, created_at:TIMESTAMP",
        timePartitioning: "created_at",
      });
      ...
      await this.createTableIfNotExists(dataset, "bot_metrics", {
        schema:
          "user_id:STRING, tasks_completed:INTEGER, time_saved_hours:NUMERIC, workflows_run:INTEGER, timestamp:TIMESTAMP",
        timePartitioning: "timestamp",
      });
  ...
  async logAutomation(userId, automationType, trigger, action, status) {
    try {
      ...
      await table.insert(row);
      if (this._shouldLog)
        console.log(`✅ Automation logged: ${automationType}`);
    } catch (error) {
  ...
  async syncStripePayment(stripeEvent) {
    try {
      ...
      if (this._shouldLog)
        console.log(`✅ Payment synced to BigQuery: ${stripeEvent.id}`);
      return true;
    } catch (error) {
  ...
  startAutoSync() {
    ...
    setInterval(
      () => {
        if (this._shouldLog)
          console.log("🔄 Running Stripe → BigQuery sync...");
    ...
    setInterval(
      () => {
        if (this._shouldLog)
          console.log("💾 Running daily Firestore backup...");
        this.runDailyBackup();
      },
      24 * 60 * 60 * 1000,
    ); // Every 24 hours
