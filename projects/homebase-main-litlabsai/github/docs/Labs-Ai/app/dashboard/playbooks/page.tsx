      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-gray-400">
            Battle-tested strategies from 2,800+ beauty professionals.
            Copy-paste and start winning today.
          </p>
        </div>
          ...
          {filtered.map((playbook) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${categoryBadgeColor(playbook.category)}`}
                >
                  {playbook.category.charAt(0).toUpperCase() +
                    playbook.category.slice(1)}
                </span>
                <span
                  className={`text-xs font-bold ${difficultyColor(playbook.difficulty)}`}
                >
                  {playbook.difficulty.toUpperCase()}
                </span>
              </div>

              <h3 className="font-black text-lg mb-2 group-hover:text-[#ff006e] transition">
                {playbook.title}
              </h3>
