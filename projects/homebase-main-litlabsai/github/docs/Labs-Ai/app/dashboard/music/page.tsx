...
export default function MusicPage() {
  const [activeTab, setActiveTab] = useState<
    "recommend" | "trending" | "library"
  >("recommend");
  const [isLoading, setIsLoading] = useState(false);
  const [recommendations, setRecommendations] = useState<MusicTrack[]>([]);
  const [contentDescription, setContentDescription] = useState("");
  const [platform, setPlatform] = useState<
    "tiktok" | "reel" | "story" | "youtube"
  >("tiktok");
  const [mood, setMood] = useState<
    "energetic" | "chill" | "inspiring" | "emotional" | "professional" | "fun"
  >("energetic");
    ...
    try {
      ...
      if (!response.ok) {
        if (data.upgradeRequired) {
          alert(
            "Music recommendations require the Music Add-on ($9/month). Upgrade to unlock!",
          );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/60 mt-1">
              AI-powered music recommendations for your content
            </p>
          </div>
            ...
            {recommendations.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-xl font-bold text-white">
                  Perfect Tracks for Your Content
                </h2>
