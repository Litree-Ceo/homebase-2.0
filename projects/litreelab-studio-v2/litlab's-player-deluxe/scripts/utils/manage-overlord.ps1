param(
    ...
    [ValidateSet("start", "stop", "status", "restart", "god-mode")]
...
$SocialServeEntry = Join-Path $RootDir "modules\social\node_modules\serve\build\main.js"
...
if (-not $NodePath) {
    Write-Warning "node not found on PATH. Social service cannot be started from this script."
}
elseif (-not (Test-Path $SocialServeEntry)) {
    Write-Warning "serve not installed for Social (missing node_modules). Run: cd modules\social; npm install"
}
...
$Services = @(
    @{
        ...
        Dir  = "modules\social"
        Exe  = $NodePath
        Args = @($SocialServeEntry, "-s", ".", "-l", "3000")
    },
    @{
        Name = "Grid"
        Port = 5000
...
function Show-Status {
    ...
    foreach ($Svc in $Services) {
        ...
        if ($net) {
            ...
            Write-Host "   🔗 http://$($ip):$($Svc.Port)" -ForegroundColor Cyan
        } else {
...
if ($Action -eq "god-mode") {
    Write-Host "`n⚡ GOD MODE STARTUP SEQUENCE INITIATED ⚡" -ForegroundColor Yellow
    $Action = "restart"
}
